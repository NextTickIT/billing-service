...
describe('planRetry (FR-005: days 0/1/3/5/7 from first failure)', () => {
  test('the first failure schedules the day-1 retry', () => {
    ...
    expect(plan).toEqual({ final: false, attempt: 1, nextPaymentDate: day(1) });
  });

  test('subsequent failures walk 3, 5, 7', () => {
    expect(planRetry(1, firstFailure).nextPaymentDate).toEqual(day(3));
    expect(planRetry(2, firstFailure).nextPaymentDate).toEqual(day(5));
    expect(planRetry(3, firstFailure).nextPaymentDate).toEqual(day(7));
  });
