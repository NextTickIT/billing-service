...
20. `src/modules/health/`: `routes.ts` — the module's **autoloaded entrypoint** (a Fastify route plugin: `GET /health` → `{ status: 'ok' }`), importing `domain.ts` (`checkHealth`, an Effect-returning service function, no class). Health is **route-only: no `data-access.ts`, no `.plugin`** (it subscribes to no tasks). A module adds `data-access.ts` when it needs persistence and a `*.plugin.ts` only when it subscribes to task/queue events.
21. `src/app.ts`: Fastify factory with **two scoped autoload passes**. Pass 1: `register(autoload, { dir: 'plugins', matchFilter: isPluginFile })` (system plugins). Pass 2: `register(autoload, { dir: 'modules', maxDepth: 2, dirNameRoutePrefix: false, matchFilter: (p) => isRouteFile(p) || isPluginFile(p) })` — loads each module's `routes.ts` entrypoint plus any optional `*.plugin.ts`; **`domain.ts` / `data-access.ts` are never autoloaded** (plain imports). `src/main.ts`: build app, listen on configured host/port. `src/worker.ts`: build the runtime and start the `TaskRegistry` poll-loop skeleton (logs "worker started", no real consume).
...
| `@fastify/autoload` loads non-entrypoint files (domain/data-access) as plugins | boot error / duplicate route decoration | **(fixed)** scope autoload with `matchFilter`: system pass loads `*.plugin.ts`; module pass loads `routes.ts` + optional `*.plugin.ts`; `domain`/`data-access` never autoloaded (step 21); asserted by health.test |
...
### Post-review revision — module-loading convention (user feedback)

Health is infrastructure, not the showcase for the full module pattern, so the convention changed:

- **`routes.ts` is the autoloaded module entrypoint** (a Fastify route plugin). `.plugin.ts` is now **optional** — present only when a module subscribes to task/queue events. `domain.ts` (services) and `data-access.ts` stay plain imports, never autoloaded. Autoload's module pass matches `routes.ts` **or** `*.plugin.ts`.
- **`health` is route-only:** `routes.ts` + `domain.ts` (a dependency-free `checkHealth` service). Removed `health/data-access.ts` and `health/health.plugin.ts`; health registers no task handler.
- **TaskRegistry kept** (service + `registerTaskHandler` decorator + worker dispatcher) as a documented, ready-to-use mechanism with no example consumer yet. Backend test now asserts the registry is wired but empty.
- **Clean builds:** each package's `build` now `rm -rf dist` first, because `tsc -b` incremental left an orphaned `dist/.../health.plugin.js` after the source was deleted, which autoload then loaded and crashed on. `typecheck` switched to `tsc -p tsconfig.json --noEmit` (covers `test/` too, no emit race with `build`).
