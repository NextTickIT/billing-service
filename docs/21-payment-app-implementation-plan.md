...
- **2.1 shared:** rename `packages/shared/src/schemas/subscription.ts` → `payment.ts`; barrel `index.ts:1`. Rename `Subscription`→`Payment`, `CreateSubscription`→`CreatePayment`, `SubscriptionStatus`→`PaymentStatus` (keep numeric values 0/1/2/3, `subscription.ts:48-53`). **Relocate carve-out:** `Currency`, `CurrencyCode`, `currencyFromCode`, `PaymentMethod`, `PaymentMethodSchema`, `RETRY_SCHEDULE_DAYS` currently live in `subscription.ts` and are imported cross-slice by `checkout.ts:3` and `event.ts:3`. Decide: keep them re-exported from `payment.ts` (minimal) OR split into `schemas/currency.ts`/`schemas/method.ts` (cleaner per docs/16 conv.6). **Recommend: keep in `payment.ts` for this change** to bound scope; note the split as a follow-up (open question OQ-1).
...
- **2.3 events/scheduler:** `billing/scheduler.ts`, `billing/events.ts`, `subscription/{domain,matcher,retry,period,cancel}.ts` → `payment/…`; update imports. **Wire-contract carve-out:** leave the event *names* `subscription_created`/`subscription_cancelled` and the `SUBSCRIPTION_CANCEL`/`PAYMENT_REBIND` message-type constants **unchanged** unless the sink contract is intentionally versioned (DriverB) — flag as OQ-2. Internal helper prefixes like `evt_sub_` (`events.ts:52,70`) are opaque ids; keep for idem stability. **`sub_` coupling (F-F fix):** the recurring `orderReference = sub_${id}_…` is minted at `scheduler.ts:41` AND parsed by the matcher regex `OUR_REF = /^sub_([0-9a-fA-F-]+)_/` at `subscription/matcher.ts:7,22`. Extract the `sub_` prefix into ONE shared constant used by both the mint and the match site so a future rename can't silently break self-charge matching (every self-charge would quarantine, AC-6 counter climbs). Keep the literal value `sub_` unchanged.
- **2.4 migration:** `0009_rename_payments.ts`: first `ALTER TABLE payments RENAME TO payment_fixations` (the fixation table from `0004`), then `ALTER TABLE subscriptions RENAME TO payments`; rename `subscriptions_one_active_per_user`/`subscriptions_due_idx` indexes (`0006:32-39`); rename `payment_fixations."subscriptionId"`→`"paymentId"` and `quarantine_records."boundSubscriptionId"`→`"boundPaymentId"` and the checkout `subscriptionId` refs. Preserve all data (P1, AC-6). *(Naming note: if `payment_fixations` is undesired, choose `charge_fixations`; align with the Charge model. Resolve in review — OQ-3.)*
- **2.5 second grep gate (F-G fix — the silent-corruption path).** The **charge** module already SELECTs/INSERTs the OLD `payments` *fixation* table (`payments/data-access.ts:116-120` `INSERT INTO payments (...)`, `pipeline.e2e.ts:202` `FROM payments`). After 2.4 renames that fixation table to `payment_fixations` AND renames `subscriptions`→`payments`, every `FROM payments`/`INTO payments` literal in the charge module must be repointed to `payment_fixations` — otherwise it **silently reads the new recurring-Payment table** (no compile error, data corruption). Gate: after Phase 2, `FROM payments`/`INTO payments` in `charge/data-access.ts` (and its e2e) = 0; they must all read `payment_fixations`.
- **Acceptance:** grep `Subscription|subscriptions` in `src` = 0 (AC-1); the 2.5 charge-module `payments`→`payment_fixations` gate passes; typecheck/lint/test green; e2e checkout→payment row in `payments` table AND a charge fixation row in `payment_fixations`.
...
- **3.3** New `quarantine/routes.ts`: `GET /api/quarantine` (from `support:listOpenQuarantine`, `support/routes.ts:131-140`) and `POST /api/quarantine/:id/bind` (from `support` bind, `routes.ts:88-126`, keeps `insertAudit` + `enqueue(PAYMENT_REBIND)`). Preserve audit logging (docs/19 §4 "audited"; `support/routes.ts:106-112`). Deliveries route (`support/routes.ts:142-153`) — **park behind operator auth, do NOT delete (F-I fix)**: it is the only operator read of undelivered-events, an NFR operator-queue/alert surface (CLAUDE.md §10). docs/19 §6/§11 defers the dashboard *screen*, not this API. Move it to `GET /api/deliveries` (operator role) or keep under quarantine; resolve OQ-4 **before** starting Phase 3, not after.
- **3.4** Payment **detail** must list the Payment's Charges (AC-4): add a repo read joining `charges` (and/or `payment_fixations`) by `externalUserId`/`paymentId`; return the date model + Charges array (docs/19 §6).
...
- **5.2 public checkout JSON read** (docs/19 §11.6, AC-9/10/12): replace the HTML stub `fastify.get('/checkout/:id')` (`checkout/routes.ts:155-167`, returns `pageHtml`) with a **public JSON** read returning `{ amount, currency, period, status, expiresAt }` (no `externalUserId` — AC-9). Keep `GET /checkout/:id` unauthenticated (unguessable `chk_` id) and add/keep the public `POST …/pay` returning the signed form. Change `checkoutPath` (`checkout/domain.ts:14`) and any return-URL usage from `/checkout/:id` to `/c/:id` to match the SPA (docs/19 §7).
- **5.3 shared-secret middleware — explicit allowlist, NOT a blanket `/api/*` (F-C fix).** A blanket `/api/*` pre-handler self-DoSes real traffic: the provider callback `POST /api/providers/:provider/callback` (`checkout/routes.ts:177-183`) is **under `/api/`** and WayForPay won't send the secret; and `POST /api/checkout-sessions` (create) is called by **SendPulse directly, not via the BFF** (docs/19 §5). Gate the secret ONLY on the BFF-proxied routes: **gate** `{/api/payment*, /api/quarantine*, the public checkout session-read + `…/pay` (which go through the BFF)}`; **exclude** `{/api/providers/*/callback (signature-verified), POST /api/checkout-sessions create (Service token, direct SendPulse), /health}`. Implement as an explicit route allowlist/tag, not a path-prefix hook. Point `W4P_RETURN_URL` (`config.ts:101`) at `checkout.nexttick.it/c/:id/return`.
...
- **6.5 checkout module (AC-9/10/11/12):** `/c/:id` renders amount+currency+period (JSON read via BFF), single "Pay by card" → `POST …/pay` → auto-submit the WayForPay form (`{action,fields}` shape from `wayforpay/purchase.ts:39-55`). `/c/:id/return` polls session status until `completed`/timeout/declined. Expired/completed → terminal state, not a form.
- **6.6 operator console (AC-14/15):** `session/` login (`/ops/login`) → BFF sets Secure HttpOnly cookie; router guard on `/ops/*` → redirect to login without cookie. `payments/` filter by `externalUserId` → detail (Charges) → cancel + create. `quarantine/` list → bind.
- **6.7 BFF (AC-13/14/19):** `functions/api/` Pages Functions proxy to `BACKEND_ORIGIN` with the shared-secret header; cookie→operator-token injection for `/ops` `/api/*`; NO cookie → 401 without calling backend; secrets from Pages env, not committed. SPA deep links resolve on refresh.
...
- Update `CLAUDE.md` §6 (entity list `Subscription`→`Payment`, "one active subscription"→"one active Payment"), §8/§9 (routes: no `/support`; `/api/payment` + `/api/quarantine`), §5 (table names `subscriptions`→`payments`, `incoming_payment_events`→`charges`; note `payments`→`payment_fixations`/`charge_fixations`).
...
| AC-9 `/c/:id` renders amount/currency/period, no subscriber data | Ph6 checkout module test + JSON read excludes `externalUserId` |
...
| AC-14 `/ops/*` cookie-guarded; HttpOnly; no token in storage | Ph6 router guard + BFF cookie test |
...
**Consequences.** Two large but green-between atomic side-renames; one denormalized `nextPaymentDate` to keep consistent with the anchor; wire event names and idem-key literals deliberately unchanged (opaque transport stability) — the entity rename does NOT propagate to the sink vocabulary in this change; the `payments` fixation table gets a new name (`payment_fixations`/`charge_fixations`) to free `payments` for the recurring entity.

**Follow-ups.** OQ-1 split `Currency`/`PaymentMethod` into own slices; OQ-2 version the sink event vocabulary (`subscription_*`→`payment_*`); OQ-3 final fixation-table name; OQ-4 deliveries route disposition; re-evaluate 2B if the console needs richer schedule projection.

---

## 12. Open questions (could not be resolved from code — persist to open-questions.md)
- **OQ-1** Relocate `Currency`/`CurrencyCode`/`PaymentMethod`/`RETRY_SCHEDULE_DAYS` out of the renamed slice into `schemas/currency.ts`/`schemas/method.ts`? (docs/16 conv.6 suggests yes; scope trap now.) They are cross-imported by `checkout.ts:3` and `event.ts:3`.
- **OQ-2** Do we rename the external sink event names `subscription_created`/`subscription_cancelled` → `payment_*`? This is a breaking wire-contract change to sinks (`event.ts:16-24`). docs/19 §12 says update docs but is silent on the wire vocabulary. Recommend: keep wire names for now; version later.
- **OQ-3** Final name for the old `payments` fixation table (`payment_fixations` vs `charge_fixations`)? (docs/19 doesn't name it; it must move to free `payments`.)
- **OQ-4** Keep, park, or delete the operator "deliveries" route (`support/routes.ts:142-153`)? docs/19 §6/§11 defers deliveries.
- **OQ-5** Does "create Payment" require amount/period up front, or can it be created with only `externalUserId` and filled on first matched Charge? (docs/19 §14 open; §4 implies amount/period/method.)
- **OQ-6** Final public domain (`checkout.nexttick.it` assumed) + Cloudflare account/domain registration + whether crypto/Whitepay appears on checkout (docs/19 §14). Card-only for MVP (CLAUDE.md §8).
...
- **F-G (MAJOR-impact, silent)** Phase 2.5 adds the second grep gate: charge-module `FROM/INTO payments` → `payment_fixations` (else it silently reads the new recurring table).
...
- **F-I (MINOR)** Phase 3.3 parks the deliveries route behind operator auth (NFR: undelivered-events queue) instead of deleting; resolve OQ-4 before Phase 3.
...
**Verification additions:** the migration e2e must assert `raw_events`/`attempts` row counts survive (not just Payment rows); add the two silent-corruption grep gates (F-A past_due anchor, F-G charge-module `payment_fixations`) to the per-phase checklist (§10).
